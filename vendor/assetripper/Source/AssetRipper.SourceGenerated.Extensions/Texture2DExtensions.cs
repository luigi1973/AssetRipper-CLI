...
public static class Texture2DExtensions
{
	extension(ITexture2D texture)
	{
		...
		{
			if (texture.StreamData_C28 is not null && texture.StreamData_C28.IsSet())
			{
				return texture.StreamData_C28.CheckIntegrity(texture.Collection);
			}
			else if (!texture.ImageData_C28.IsNullOrEmpty())
			{
				return true;
			}
			else
		...
		public byte[] GetImageData()
		{
			...
			{
				data = texture.ImageData_C28;
			}

			if (data.Length != 0 && swapBytes)
			{
		...
		public int ImageDataLength
		{
			get
			{
				...
				else if (texture.ImageData_C28.Length != 0)
				{
